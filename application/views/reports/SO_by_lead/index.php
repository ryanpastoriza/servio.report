		...
		<div class="box-body">
			...
			<div style="margin-bottom:15px;display:flex;align-items: top;" class="row">
				...
				<div style="display:flex;flex-direction:column;overflow:hidden" class="col-md-4">
					...
					<a href="" class="btn btn-primary" id="setBtn" style="margin-left:69px;">Search</a>
...
<script>
	...
	$(document).ready(function(){
		...
		$('title').html("Sales Order By Lead Source Report");
	});
	...
	$("#so_status").change(function(e){
		...
		let branch = $("#branch").val();
		let dealer = $("#dealer").val();
		window.location = "so_by_leads?" + 'start_date=' + startDate + "&end_date=" + endDate + "&so_status=" + e.target.value + "&branch=" + branch + "&dealer=" + dealer;
	});
	...
	$("#branch").change(function(e){
		...
		let so_status = $("#so_status").val();
		let dealer = $("#dealer").val();
		window.location = "so_by_leads?" + 'start_date=' + startDate + "&end_date=" + endDate + "&so_status=" + so_status + "&branch=" + e.target.value + "&dealer=" + dealer;
	});
	...
	let table = TableExport(document.getElementsByTagName("table"), {
		  ...
		  filename: "SO by Lead Source <?php echo date('m/d/Y') ?>",                     // (id, String), filename for the downloaded file, (default: 'id')
